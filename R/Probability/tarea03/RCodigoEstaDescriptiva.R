######### Diapositiva 9 ######
datos<-read.csv("DatosEjercicios.csv")
summary(datos[,4])
table(datos[,4])
barplot(table(datos[,4]), las=1,
        xlab="Tipo de vegetacion", ylab="Frecuencia absoluta")

######### Diapositiva 11 ######
conteo<-hist(datos[,5], breaks=6, plot=FALSE)
hist(datos[,5], breaks=6, plot=TRUE)
...
barplot(tablacontingencia, beside = TRUE, col=c("grey80","grey40"))
